                ...
                <div class="mt-5 md:mt-0 md:col-span-2">
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <select wire:model.live="property_type" id="property_type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                        <div>
                                            ...
                                            <select wire:model="new_unit_type" id="new_unit_type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                            ...
                                            @error('new_unit_type') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                        </div>
                                        
                                        <div>
                                            <label for="new_unit_status" class="block text-sm font-medium text-gray-700">Status</label>
                                            <select wire:model="new_unit_status" id="new_unit_status" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                <option value="">Select Status</option>
                                                <option value="available">Available</option>
                                                <option value="occupied">Occupied</option>
                                                <option value="maintenance">Maintenance</option>
                                            </select>
                                            @error('new_unit_status') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                        </div>
                                        
                                        <div>
                                            <label for="new_unit_amount" class="block text-sm font-medium text-gray-700">Number of Units</label>
                                            <input type="number" wire:model="new_unit_amount" id="new_unit_amount" min="1" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                            @error('new_unit_amount') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                        </div>

                                        <div>
                                            <label for="new_unit_description" class="block text-sm font-medium text-gray-700">Unit Name/Description</label>
                                            <input type="text" wire:model="new_unit_description" id="new_unit_description" placeholder="e.g., 2 Bedroom, 2 Bath" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                            @error('new_unit_description') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                        </div>
                                            ...
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                                @foreach($availableAmenities as $amenity)
                                                    <div class="flex items-center">
                                                        <input type="checkbox" wire:model="new_unit_amenities" value="{{ $amenity }}" id="new_unit_amenity_{{ $amenity }}" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                                                        <label for="new_unit_amenity_{{ $amenity }}" class="ml-2 block text-sm text-gray-900">
                                        ...
                                        <div class="flex items-end">
                                            <button type="button" wire:click="addUnit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                                Add Unit
                                            </button>
                                        </div>
                                    </div>
                                                        ...
                                                        <li class="px-4 py-3">
                                                            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                                                <div>
                                                                    ...
                                                                    <select wire:model.live="unit_list.{{ $index }}.type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                                        @if($property_type == 'residential' || $property_type == 'mixed')
                                                                            <optgroup label="Residential">
                                                                                <option value="apartment">Apartment</option>
                                                                                <option value="house">House</option>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <select wire:model.live="unit_list.{{ $index }}.status" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                                        <option value="available">Available</option>
                                                                        <option value="occupied">Occupied</option>
                                                                        <option value="maintenance">Maintenance</option>
                                                                    </select>
                                                                ...
                                                                <div>
                                                                    <label class="block text-sm font-medium text-gray-700">Unit Name/Description</label>
                                                                    <input type="text" wire:model.live="unit_list.{{ $index }}.description" placeholder="e.g., 2 Bedroom, 2 Bath" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                                </div>
                                                                
                                                                <div class="flex items-end">
                                                                    <button type="button" wire:click="removeUnit({{ $index }})" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                                                        Remove
                                                                    </button>
                                                                </div>
                                                                ...
                                                                <div class="col-span-5">
                                                                    ...
                                                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                                                        @foreach($availableAmenities as $amenity)
                                                                            <div class="flex items-center">
                                                                                <input type="checkbox" wire:model.live="unit_list.{{ $index }}.amenities" value="{{ $amenity }}" id="unit_{{ $index }}_amenity_{{ $amenity }}" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                                                                                <label for="unit_{{ $index }}_amenity_{{ $amenity }}" class="ml-2 block text-sm text-gray-900">
                                                                                    {{ ucwords(str_replace('_', ' ', $amenity)) }}
                                                                                </label>
                                                                            </div>
                                                                        @endforeach
                        ...
                        <div class="col-span-6 sm:col-span-2">
                            ...
                            <input disabled type="number" wire:model="total_units" id="total_units" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        ...
                        <div class="col-span-6 sm:col-span-2">
                            ...
                            <input disabled type="number" wire:model="available_units" id="available_units" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
